...
(function(){
  var saved = sessionStorage.getItem('kk_theme');
...
function setLang(l){
  ...
  // If on project-details page, re-render panels in new language
...
function toggleTheme(){
  ...
  document.body.classList.remove('t-primary','t-secondary');
  document.body.classList.add('t-'+theme);
  sessionStorage.setItem('kk_theme', theme);
}

// Scroll progress bar
var _prog = document.getElementById('progress');
if(_prog){
  window.addEventListener('scroll',function(){
    var pct=window.scrollY/(document.documentElement.scrollHeight-window.innerHeight)*100;
    _prog.style.width=pct+'%';
  },{passive:true});
}
...
// Custom cursor tracking (only on pages that have the cursor element)
...
(function () {
  ...
  document.addEventListener('click', function (e) {
    var a = e.target.closest('a[data-browser="true"]');
    if (!a) return;
